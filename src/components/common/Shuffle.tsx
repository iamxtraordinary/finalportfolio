...
try {
  const stModule = require('gsap/SplitText');
  GSAPSplitText = stModule.SplitText || stModule.default || null;
} catch (e) {}
...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  const customCharsRef = useRef<HTMLElement[]>([]);
  ...
  useGSAP(
    () => {
      ...
      const teardown = () => {
        ...
        if (customCharsRef.current.length) {
          el.textContent = text;
          customCharsRef.current = [];
        }
  ...
  const commonStyle = useMemo(
    ...
    [textAlign, fallbackFont, style, ready]
  );
